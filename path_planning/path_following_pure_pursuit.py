...
if __name__ == "__main__":
    ...
    # Controller setting
    linear_vel = config.getfloat('PURE_PURSUIT', 'linear_vel')
    max_steering_angle = config.getfloat('PURE_PURSUIT', 'max_steering_angle') * np.pi / 180
    k_p, k_d, k_i = config.getfloat('PURE_PURSUIT', 'k_p'),\
                    config.getfloat('PURE_PURSUIT', 'k_d'),\
                    config.getfloat('PURE_PURSUIT', 'k_i')
    lookahead_dist = config.getfloat('PURE_PURSUIT', 'lookahead_dist')
    ...
    while not controller.finished():
        ...
        lookahead_pt = controller.loop(dt=dt, axle_len=axle_len)
        pose = robot.get_pose()
